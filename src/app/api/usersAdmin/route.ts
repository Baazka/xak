//src/app/api/usersAdmin/route.ts
...
export const GET = withAuth(async (req: NextRequest, user: JwtPayload) => {
  ...
  if (search) {
    params.push(`%${search}%`);
    whereClause += ` AND (user_firstname ILIKE $${params.length} OR user_email ILIKE $${params.length} and org_register_no ILIKE $${params.length} and org_legal_name ILIKE $${params.length} )`;
  }
