...
export const POST = withAuth(async (req: NextRequest, user: JwtPayload) => {
  ...
  const opRawData: {
    ...
    op_name: string;
    op_date: Date;
  }[] = body.operations; // expect array of {op_id, op_form_id, op_code, op_name, op_date }
  try {
    for (const opData of opRawData) {
      const { op_id, op_code, op_name, op_date } = opData;
      if (!op_id || op_id === null) {
        await client.query(
          `INSERT INTO audit_org_operation (op_aud_id, op_form_id, op_code, op_name, op_date)
            VALUES ($1, $2, $3, $4, $5) RETURNING op_id`,
          [audId, formId, op_code, op_name, op_date]
        );
      } else {
        await client.query(
