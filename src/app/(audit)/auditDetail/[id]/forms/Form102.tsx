...
export default function Form102({ auditId, formListId }: Props) {
  ...
  const removeRow = async (typeId: number, index: number) => {
    ...
    try {
      ...
      if (row.det_id && row.det_id !== 0) {
        const res = await fetchWithAuth(`/api/audit/form102/detail/`, {
          method: "DELETE",
  ...
  const removeOPRow = async (index: number) => {
    ...
    try {
      if (row.op_id && row.op_id !== 0) {
        const res = await fetchWithAuth(`/api/audit/form102/operation/`, {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
  ...
  useEffect(() => {
    const fetchForm = async () => {
      try {
        ...
        const orgRes = await fetchWithAuth(`/api/audit/form102?aud_id=${auditId}`, {
          cache: "no-store",
        });

        const detailRes = await fetchWithAuth(`/api/audit/form102/detail?aud_id=${auditId}`, {
          cache: "no-store",
        });

        const opRes = await fetchWithAuth(`/api/audit/form102/operation?aud_id=${auditId}`, {
          cache: "no-store",
        });
  ...
  const handleSaveAll = async () => {
    try {
      ...
      const [detailRes, opRes] = await Promise.all([
        fetchWithAuth(`/api/audit/form102/detail/`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            aud_id: auditId,
        ...
        fetchWithAuth(`/api/audit/form102/operation/`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            aud_id: auditId,
